...
local function OnTargetChanged(self)
    ...
    if UnitIsUnit(unit, "target") then
        ...
        -- Target Arrow
        if cfg.units.nameplate.showTargetArrow then self.arrow:Show() end

        -- Highlight
        if cfg.units.nameplate.showHighlight then self.highlight:Show() end

        -- Glow
        if cfg.units.nameplate.showGlow then self.glow:Show() end

        -- Show Class Icons
        if self.classPower then self.classPower:Show() end
    else
        self.TargetBorder:Hide()
        if cfg.units.nameplate.showTargetArrow then self.arrow:Hide() end

        if cfg.units.nameplate.showHighlight then self.highlight:Hide() end

        if cfg.units.nameplate.showGlow then self.glow:Hide() end

        -- Hide Class Icons
        if self.classPower then self.classPower:Hide() end
    end
end
