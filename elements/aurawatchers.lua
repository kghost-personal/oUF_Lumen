...
Watchers   - A Frame to hold `Button`s representing both buffs and debuffs.
...
local function createAuraIcon(element, index)
	...
	--[[ Callback: Watchers:PostCreateIcon(button)
...
local function updateIcon(element, unit)
	...
	while(true) do
		...
		timeMod, effect1, effect2, effect3 = UnitAura(unit, index)
		...
		if(position and (caster == 'player' or caster == 'vehicle')) then
			...
			--[[ Callback: Watchers:PostUpdateIcon(unit, button, index, position)
	...
	for spellId, index in pairs(element.Watchers) do
		if (exists[index] == nil) then
			element.Icons[index]:Hide()
		end
	end
end
...
local function UpdateWatchers(self, event, unit)
	...
	local auras = self.Watchers
	if(auras) then
		--[[ Callback: Watchers:PreUpdate(unit)
		...
		* unit - the unit for which the update has been triggered (string)
		...
		updateIcon(auras, unit)

		--[[ Callback: Watchers:PostUpdate(unit)
...
local function Update(self, event, unit)
	...
	if(event == 'ForceUpdate' or not event) then
		local auras = self.Watchers
		if(auras) then
			for spellId, index in pairs(auras.Watchers) do
				(auras.SetPosition or SetPosition) (auras, index)
			end
		end
	end
end
...
local function Enable(self)
	if(self.Watchers) then
		...
		if(auras) then
			...
			auras.Icons = {}
			for spellId, index in pairs(auras.Watchers) do
				--[[ Override: Watchers:CreateIcon(position)
				...
				* button - the button used to represent the aura (Button)
				--]]
				auras.Icons[index] = (auras.CreateIcon or createAuraIcon) (auras, index)

				--[[ Override: Watchers:SetPosition(from, to)
				...
				* to   - the offset of the last aura button to be (re-)anchored (number)
				--]]
				local f = (auras.SetPosition or SetPosition)
				f(auras, index)
			end
...
local function Disable(self)
	if(self.Watchers) then
		...
		if(self.Watchers) then self.Watchers:Hide() end
	end
end

oUF:AddElement('Watchers', Update, Enable, Disable)
